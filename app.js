...
function forumHTML(course,threads){
  ...
  return `<div class="forum-layout"><div>
    ${threads.length?threads.map(t=>`<article class="forum-thread">
      ...
      ${isStaff()?`<form class="reply-form" data-thread="${t.id}" data-course="${course.id}"><textarea name="body" required placeholder="Responder como ${roleName(state.profile.role)}..."></textarea><button class="primary mini">Responder</button></form>`:""}
    ...
    ${student?`<h3>Nueva consulta</h3><p style="font-size:.8rem;color:var(--muted)">Podés hacer una pregunta o compartir un enlace para que responda el equipo docente.</p><form id="newThreadForm" data-course="${course.id}">
      <div class="field"><label>Título</label><input name="title" required maxlength="160"></div><div class="field"><label>Pregunta / comentario</label><textarea name="body" required></textarea></div><div class="field"><label>Enlace opcional</label><input name="link_url" type="url" placeholder="https://..."></div><button class="primary" style="width:100%">Publicar consulta</button></form>`
      :`<h3>Foro del curso</h3><p style="font-size:.82rem;color:var(--muted)">Los alumnos pueden publicar preguntas o links. Profesores y administradores responden desde cada consulta.</p>`}
  </div></aside></div>`;
}
...
async function lessonHTML(cid,lid){
  ...
  let body=lesson.content_type==="video"?`<iframe class="video-frame" src="${esc(youtubeEmbed(lesson.content_url||""))}" allowfullscreen></iframe>`:lesson.content_type==="text"?`<div class="material-box" style="text-align:left;white-space:pre-wrap">${esc(lesson.text_content||"")}</div>`:`<div class="material-box"><h3>${esc(lesson.title)}</h3><p>${esc(lesson.description||"")}</p><a class="primary" style="display:inline-block;text-decoration:none" href="${esc(lesson.content_url||"#")}" target="_blank" rel="noopener">Abrir material ↗</a></div>`;
...
function lessonForm(l={}){return `<div class="field"><label>Título</label><input name="title" required value="${esc(l.title||"")}"></div><div class="field"><label>Descripción</label><textarea name="description">${esc(l.description||"")}</textarea></div><div class="field"><label>Tipo</label><select name="content_type"><option value="video" ${l.content_type==="video"?"selected":""}>Video de YouTube</option><option value="drive" ${l.content_type==="drive"?"selected":""}>Drive</option><option value="link" ${l.content_type==="link"?"selected":""}>Enlace</option><option value="text" ${l.content_type==="text"?"selected":""}>Texto</option></select></div><div class="field"><label>URL</label><input name="content_url" type="url" value="${esc(l.content_url||"")}"></div><div class="field"><label>Texto (si corresponde)</label><textarea name="text_content">${esc(l.text_content||"")}</textarea></div><div class="field"><label>Orden</label><input name="position" type="number" min="1" value="${Number(l.position||1)}"></div>`;}
...
function bindContent(){
  ...
  if(upload)upload.onsubmit=async e=>{e.preventDefault();const [cid,mid]=document.getElementById("upTarget").value.split("|");const mods=await loadCourseModules(cid),m=mods.find(x=>String(x.id)===String(mid));const payload={module_id:Number(mid),title:document.getElementById("upTitle").value.trim(),description:document.getElementById("upDesc").value.trim(),content_type:document.getElementById("upType").value,content_url:document.getElementById("upUrl").value.trim(),position:(m?.lessons?.length||0)+1,created_by:state.user.id};const {error}=await sb.from("lessons").insert(payload);if(error){toast(error.message,true);return;}delete state.modules[cid];toast("Contenido publicado");upload.reset();};
  ...
  document.querySelectorAll(".reply-form").forEach(f=>f.onsubmit=async e=>{e.preventDefault();const body=new FormData(e.target).get("body");const courseId=Number(e.target.dataset.course);const {error}=await sb.from("forum_replies").insert({thread_id:Number(e.target.dataset.thread),user_id:state.user.id,body});if(error){toast("No se pudo responder: "+error.message,true);return;}delete state.forum[courseId];toast("Respuesta publicada");renderShell();});
  ...
  document.querySelectorAll(".add-lesson").forEach(b=>b.onclick=async()=>{const cid=Number(b.dataset.course),mid=Number(b.dataset.module),mods=await loadCourseModules(cid),m=mods.find(x=>x.id===mid);showModal("Agregar contenido",lessonForm({content_type:"video",position:m.lessons.length+1}),async fd=>{const p=Object.fromEntries(fd);p.module_id=mid;p.position=Number(p.position);p.created_by=state.user.id;if(p.content_type!=="text"&&!p.content_url){toast("Agregá una URL",true);return false;}const {error}=await sb.from("lessons").insert(p);if(error){toast(error.message,true);return false;}delete state.modules[cid];toast("Contenido agregado");renderShell();});});
  document.querySelectorAll(".edit-lesson").forEach(b=>b.onclick=async()=>{const cid=Number(b.dataset.course),mid=Number(b.dataset.module),mods=await loadCourseModules(cid),m=mods.find(x=>x.id===mid),l=m.lessons.find(x=>String(x.id)===b.dataset.lesson);showModal("Editar contenido",lessonForm(l),async fd=>{const p=Object.fromEntries(fd);p.position=Number(p.position);const {error}=await sb.from("lessons").update(p).eq("id",l.id);if(error){toast(error.message,true);return false;}delete state.modules[cid];toast("Contenido actualizado");renderShell();});});
